...
export async function fetchForecast() {
  ...
  try {
    ...
    return {
      ...
      sunrise: hhmm(sr[1]), sunset: hhmm(ss[1]), sunriseISO: sr[1] || null, sunsetISO: ss[1] || null,
...
// NOAA SWPC aurora signals: current planetary Kp, the upcoming forecast peak Kp, and OVATION aurora
// probability at the bay and over northern Michigan. No API key. Live (Kp changes hourly).
export async function fetchAurora() {
  const out = { kpNow: null, kpPeak: null, kpPeakTime: null, ovationBay: null, ovationNorth: null };
  try {
    const k = await getJSON("https://services.swpc.noaa.gov/json/planetary_k_index_1m.json");
    if (Array.isArray(k) && k.length) {
      const last = k[k.length - 1];
      const v = last.estimated_kp != null ? last.estimated_kp : last.kp_index;
      if (v != null) out.kpNow = Math.round(v * 10) / 10;
    }
  } catch (e) {}
  try {
    const f = await getJSON("https://services.swpc.noaa.gov/products/noaa-planetary-k-index-forecast.json");
    if (Array.isArray(f)) {
      const fut = f.filter((r) => r && r.observed === "predicted" && r.kp != null);
      if (fut.length) {
        let mx = fut[0];
        for (const r of fut) if (r.kp > mx.kp) mx = r;
        out.kpPeak = Math.round(mx.kp * 10) / 10;
        out.kpPeakTime = mx.time_tag;
      }
    }
  } catch (e) {}
  try {
    const ov = await getJSON("https://services.swpc.noaa.gov/json/ovation_aurora_latest.json");
    const coords = ov && ov.coordinates ? ov.coordinates : [];
    let bayBest = null, bayD = 1e9, north = 0;
    for (const c of coords) {
      const lo = c[0], la = c[1], v = c[2];
      const dd = (lo - 276.1) ** 2 + (la - 43.6) ** 2;
      if (dd < bayD) { bayD = dd; bayBest = v; }
      if (la >= 45 && la <= 48 && lo >= 271 && lo <= 281 && v > north) north = v;
    }
    out.ovationBay = bayBest;
    out.ovationNorth = north;
  } catch (e) {}
  return out;
}
