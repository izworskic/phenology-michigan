...
export const EVENTS = [
  ...
  { name: "Sandhill crane return", cat: "bird", s: 66, p: 80, e: 100, note: "Bugling back to the marshes and wet fields.", signal: "Sandhill cranes are back, their rolling bugle the surest sound of the marsh thawing; the cranes and the red-wings open spring together." },
  { name: "Sandhill crane fall staging", cat: "bird", s: 280, p: 305, e: 332, note: "Thousands gather at the Shiawassee Flats before heading south.", signal: "The great fall gathering: sandhill cranes stage by the thousands at the Shiawassee Flats and the bay marshes, one of the best wildlife shows in the state, moving out as the hard frosts set in." },
...
export const RIVERS = [
  { id: "ausable", name: "AuSable", site: "04136000", hatchOffset: 0, note: "the trout water, a trip north" },
  { id: "saginaw", name: "Saginaw River", site: "04157005", hatchOffset: 0, note: "the big river up the shoreline" },
  { id: "rifle", name: "Rifle", site: "04142000", hatchOffset: -2, note: "Trout Camp water, drains to Saginaw Bay" },
];

// Categories that actually track accumulated heat, and therefore move when the season runs
// early or late. Bird migration is driven by photoperiod and by weather far to the south, and
// lake levels by basin hydrology, so neither is shifted: a warm spring in Michigan does not
// move the equinox. Shifting everything would have been easier and wrong.
const HEAT_DRIVEN = new Set(["hatch", "bloom", "garden", "wild", "fish"]);

// classify events relative to a day of year, with a per-river offset on hatches and an optional
// whole-season offset from the banked record (see seasonOffsetDays)
export function classify(doy, hatchOffset = 0, seasonOffset = 0) {
  const active = [], imminent = [], recent = [];
  EVENTS.forEach((ev) => {
    const season = HEAT_DRIVEN.has(ev.cat) ? -seasonOffset : 0;
    const off = (ev.cat === "hatch" ? hatchOffset : 0) + season;
...
// ---------------------------------------------------------------- the season's own clock
//
// Leopold's phenology was not a calendar of what happens. It was a RECORD of when things
// actually happened, kept year on year, and its value was that it revealed something the
// calendar could not: whether the season itself was early or late. His family's notes are
// read by climate scientists today for exactly that reason.
//
// The banked record already carried the two numbers needed for this and never said anything
// with them. Every snapshot holds the accumulated growing degree days for the year and the
// normal accumulation for that date. The difference is usually shown as a heat number, which
// means nothing to anyone standing in a garden. Converted against the normal curve it becomes
// the thing people actually want to know: this year is running N days behind, or ahead.
//
// A day count is honest here in a way a heat count is not. Saying the year is eight days behind
// means the lilacs, the hatches and the last frost are all likely to run about eight days late,
// and that is a claim the reader can go outside and check.
export function seasonOffsetDays(gdd, doy) {
  if (!Number.isFinite(gdd) || !Number.isFinite(doy)) return null;
  const series = gddSeries();
  // Which day of the normal year first reaches the heat this year has actually accumulated.
  let matched = null;
  for (const p of series) {
    if (p.gdd >= gdd) { matched = p.doy; break; }
  }
  if (matched === null) matched = 365; // hotter than any normal day; the year is far ahead
  // Positive means ahead of normal, negative means behind.
  const offset = matched - doy;
  // Early in the year a few degree days swing the answer wildly, so it is not reported until
  // enough heat has accumulated for the number to mean anything.
  if (gdd < 120) return null;
  return -offset === 0 ? 0 : offset;
}

// Plain words for the offset. Under three days is not a meaningful difference in a season and
// should not be dressed up as one.
export function seasonOffsetPhrase(days) {
  if (days === null || days === undefined) return null;
  const n = Math.abs(days);
  if (n < 3) return "running close to normal";
  const unit = n === 1 ? "day" : "days";
  return days > 0 ? `running about ${n} ${unit} ahead of normal` : `running about ${n} ${unit} behind normal`;
}

// ---------------------------------------------------------------- the turning year
//
// Part I of A Sand County Almanac is an almanac in the plain sense: twelve months, each with its
// own character, on one piece of ground. This site had 71 dated events and showed them only as
// now, soon, and just past, which is a dashboard. A dashboard tells you where you are. An almanac
// tells you where you are in something larger.
//
// The months are deliberately uneven. May carries fifteen events and January carries two. That is
// not a gap in the data, it is what the year is actually like here, and Leopold's January essay is
// about a thaw and a skunk track because that is all January offers. Do not pad the thin months.
export const MONTHS = Object.freeze([
  { m: 1, name: "January", line: "The lakes sit at their yearly low and the great horned owls are already on eggs. The first insistence that something has begun while everything looks finished." },
  { m: 2, name: "February", line: "Bucks drop their antlers into the snow and the eagles settle onto nests. Nothing is green, but the year has committed itself." },
  { m: 3, name: "March", line: "Sap runs in the maples, red-wings come back to the cattails, and the woodcock begins his sky dance at dusk over wet ground. The first month you can hear the change." },
  { m: 4, name: "April", line: "Ice leaves the inland lakes, steelhead push up the tributaries, and the Hendricksons come off the AuSable. On the bay the northeast wind season opens." },
  { m: 5, name: "May", line: "The crowded month, and the one that rewards attention most. Lilac, morels, warblers, apple blossom, the median last frost, and oak leaves reaching the size of a squirrel's ear." },
  { m: 6, name: "June", line: "Fawns in the tall grass, fireflies over the ditches, and the Hex hatch on the AuSable, which is the one night of the year the big trout feed without caution." },
  { m: 7, name: "July", line: "The bay comes to its warmest and its calmest, the lakes to their seasonal peak. Basswood in bloom, and the year holding still for a few weeks." },
  { m: 8, name: "August", line: "Trico spinners at first light, katydids after dark, and the first goldenrod along the roadsides. The turn begins well before anyone admits it." },
  { m: 9, name: "September", line: "Velvet comes off the antlers, the warblers go back south, and the sugar maples show their first color in the crowns." },
  { m: 10, name: "October", line: "Peak color through the northeast, the first hard frost on the garden, and trout moving onto the spawning gravel. The walleye feed up for winter." },
  { m: 11, name: "November", line: "The rut, the cranes staging, the swans going south. The last of the moving, and then the woods go quiet." },
  { m: 12, name: "December", line: "Ice takes the inland lakes, and in some years the snowy owls come down out of the north. The year closes as quietly as it opened." },
]);

// Which calendar month a day of year falls in, 1 to 12. Non leap-year reckoning, matching the
// day-of-year windows the events are written against.
...
// The year grouped into months, each event filed under the month its peak falls in, so a thing
// that spans a month boundary appears once, where it is most itself.
export function turningYear() {
  return MONTHS.map((mo) => ({
    ...mo,
    events: EVENTS
      .filter((ev) => monthOfDoy(ev.p) === mo.m)
      .sort((a, b) => a.p - b.p)
      .map((ev) => ({ ...ev, peakLabel: doyToLabel(ev.p), windowLabel: `${doyToLabel(ev.s)} to ${doyToLabel(ev.e)}` })),
  }));
}
