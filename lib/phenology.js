...
// classify events relative to a day of year
export function classify(doy) {
  const active = [], imminent = [], recent = [];
  EVENTS.forEach((ev) => {
    const s = ev.s % 365, e = ev.e % 365;
    const within = ev.e > 365 ? (doy >= ev.s || doy <= e) : (doy >= s && doy <= e);
    if (within) active.push(ev);
    else if (ev.s - doy > 0 && ev.s - doy <= 21) imminent.push(ev);
    else if (doy - ev.e > 0 && doy - ev.e <= 12) recent.push(ev);
  });
