...
// Garden crops with the soil temperature (deg F at planting depth) they want before sowing or
// transplanting, and whether they are frost sensitive. Soil-temp minimums are standard guidance.
export const GARDEN = [
  { name: "Peas", soilF: 40, frost: false, act: "sow", note: "frost hardy, among the first in" },
  { name: "Spinach and lettuce", soilF: 40, frost: false, act: "sow", note: "cool-season greens" },
  { name: "Radish and arugula", soilF: 40, frost: false, act: "sow", note: "quick and frost hardy" },
  { name: "Onions and kale", soilF: 45, frost: false, act: "plant", note: "hardy sets and transplants" },
  { name: "Carrots and beets", soilF: 45, frost: false, act: "sow", note: "direct sow, frost tolerant" },
  { name: "Potatoes", soilF: 48, frost: false, act: "plant", note: "tops are tender but plant early" },
  { name: "Sweet corn", soilF: 55, frost: true, act: "sow", note: "wants warm soil to germinate" },
  { name: "Beans", soilF: 57, frost: true, act: "sow", note: "rot in cold ground" },
  { name: "Cucumber and squash", soilF: 60, frost: true, act: "sow", note: "warm-season vine crops" },
  { name: "Tomatoes and peppers", soilF: 60, frost: true, act: "transplant", note: "warm soil and no frost" },
  { name: "Basil", soilF: 60, frost: true, act: "transplant", note: "sulks below 50 at night" },
  { name: "Melons", soilF: 65, frost: true, act: "sow", note: "the last to go in" },
];
...
export function gardenWindow(soilF, frostClear, lastFrostPassed) {
  if (soilF == null) return [];
  const out = [];
  for (const c of GARDEN) {
    const gap = c.soilF - soilF;
    if (gap > 8) continue;
    if (gap > 0) {
      out.push({ name: c.name, act: c.act, note: c.note, ready: false, status: `soil ${Math.round(gap)} deg too cold`, sort: 2 });
      continue;
    }
    const blocked = c.frost && !(frostClear && lastFrostPassed);
    out.push({ name: c.name, act: c.act, note: c.note, ready: !blocked, status: blocked ? "soil ready, wait on frost" : `${c.act} now`, sort: blocked ? 1 : 0 });
  }
  out.sort((a, b) => a.sort - b.sort);
  return out;
}
