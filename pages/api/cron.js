...
import { dayOfYear, gddSeries, seasonOf, cToF, moonPhase, EVENTS, seasonOffsetDays, seasonOffsetPhrase } from "../../lib/phenology";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const snap = {
      ...
      seasonOffsetDays: seasonOffsetDays(gdd?.total ?? null, doy),
    ...
    if (!historyConfigured()) {
      ...
      const offDays = seasonOffsetDays(snap.gdd, snap.doy);
      const note = await writeAlmanac(snap, inWindow, soonEvents, seasonOffsetPhrase(offDays));
      if (note.ok) {
